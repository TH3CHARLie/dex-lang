...
module Imp (toImpModule, getIType, impBlockType, impFunType,
            toScalarType, fromScalarType, impInstrTypes,
            impModuleMainFunType) where
...
data ImpCatEnv n = ImpCatEnv
  { envPtrsToFree :: [IExpr n]
  , envScope      :: Scope n
  , envDecls      :: [ImpDecl n ()]
  , envFunctions  :: [(Name n, ImpFunction n)]}

type ImpM n = ExceptT () (ReaderT ImpCtx (Cat (ImpCatEnv n)))
type AtomRecon = Abs (Nest Binder) Atom

toImpModule :: Bindings n -> Backend -> CallingConvention
            -> Maybe (Dest n) -> Abs IBinderList Block n
            -> (ImpModule n, AtomRecon n)
...
requiredFunctions :: HasVars e => Scope n -> e n -> [(Name n, Atom n)]
...
translateTopLevel :: Bindings n -> WithDest n (Block n) -> ImpM n (AtomRecon n, [IExpr n])
...
runImpM :: ImpCtx -> Scope n -> ImpM n a -> a
...
-- toImpStandalone :: Name -> Atom -> ImpM n ImpFunction
...
-- translateDecl :: SubstEnv i n -> WithDest Decl -> ImpM n SubstEnv
-- translateDecl env (maybeDest, (Let _ b bound)) = do
--   b' <- traverse (impSubst env) b
--   ans <- translateExpr env (maybeDest, bound)
--   return $ b' @> ans

translateExpr :: SubstEnv i n -> WithDest n (Expr i) -> ImpM n (Atom n)
translateExpr env (maybeDest, expr) = case expr of
  ...
  --           translateBlock mempty (maybeDest, withoutArrow $ applyAbs a x)
  --         _ -> error $ "Invalid Imp atom: " ++ pprint f
      ...
      --   copyAtom argDest x
      --   void $ emitStatement $ ICall (fname:>LiftH iFunTy) allElts
  ...
  Case e alts _ -> do
    ...
    case e' of
      DataCon _ _ con args -> do
        case alts !! con of
          Abs bs body -> translateBlock (env <>> bs @@> args) (maybeDest, body)
      Variant (NoExt types) label i x -> do
        let LabeledItems ixtypes = enumerate types
        let index = fst $ ixtypes M.! label NE.!! i
        case alts !! index of
          Abs bs body -> translateBlock (env <>> bs @@> [x]) (maybeDest, body)
      Con (SumAsProd _ tag xss) -> do
        ...
        emitSwitch tag' $ flip map (zip xss alts) $
          \(xs, Abs bs body) ->
             void $ translateBlock (env <>> bs @@> xs) (Just dest, body)
        destToAtom dest
      _ -> error $ "Unexpected scrutinee: " ++ pprint e'

emitFunction :: RawName -> CallingConvention
             -> Abs IBinder ImpBlock n -> ImpM n (IFunVar n)
...
emitFFIFunction :: String -> [IType] -> [IType] -> ImpM n (IFunVar n)
...
impSubst :: Subst e => SubstEnv i n -> e i -> ImpM n (e n)
impSubst = undefined
-- impSubst env x = do
--   scope <- variableScope
--   return $ subst (env, scope) x

toImpOp :: WithDest n (PrimOp (Atom n)) -> ImpM n (Atom n)
toImpOp (maybeDest, op) = case op of
  ...
  PrimEffect refDest m -> do
    case m of
      ...
      --   result <- translateBlock mempty (Nothing, withoutArrow $ applyAbs f refValue)
...
toImpHof :: forall i n. SubstEnv i n -> WithDest n (Hof i) -> ImpM n (Atom n)
toImpHof env (maybeDest, hof) = do
  ...
  case hof of
    For (RegularFor d) (LamVal b body) -> do
      ...
      case idxTy of
        ...
        _ -> do
          ...
          dest <- allocDest maybeDest resultTy
          emitLoop (getRawName $ binderName b) d n \i -> do
    ...
    --     return (redKernelBody, ())
      ...
      --   combineWithDest :: Dest n -> Atom n -> ImpM n ()
      ...
      --   prevPowerOf2 :: IExpr n -> ImpM n (IExpr n)
      --   prevPowerOf2 x = do
      --     rPtr <- alloc IdxRepTy
    ...
    While (Lam (Abs b (WithArrow _ body))) -> do
      body' <- liftM snd $ scopedBlock $ do
                 ans <- translateBlock (env <>> b@>UnitVal) (Nothing, body)
                 return ((), [fromScalarAtom ans])
      emitStatement $ IWhile body'
      return UnitVal
    RunReader r (BinaryFunVal h ref _ body) -> do
      r' <- impSubst env r
      rDest <- alloc $ getType r'
      copyAtom rDest r'
      let env' = env <>> h @> undefined <>> ref @> rDest
      translateBlock env' (maybeDest, body)
    -- RunWriter (BaseMonoid e' _) (BinaryFunVal h ref _ body) -> do
...
type KernelBuilder kernel n = (ThreadInfo n -> ImpM n ()) -> ImpM n kernel
...
data DestEnv n l = DestEnv
       { _allocationInfo :: AllocInfo
       , enclosingIdxs :: IndexStructure n l
       , destDepVars :: [Name n] }
...
-- makeDest :: AllocInfo -> Type n -> Builder ([(Binder, Atom)], Dest)
-- makeDest = undefine d
...
copyDataConArgs :: EmptyNest DataConRefBinding n -> [Atom n] -> ImpM n ()
...
loadDest :: MonadBuilder n m => Dest n -> m (Atom n)
...
loadDataConArgs :: MonadBuilder n m => EmptyNest DataConRefBinding n -> m [Atom n]
...
indexDestDim :: MonadBuilder n m => Int-> Dest n -> Atom n -> m (Dest n)
...
indexDestDim d dest i = buildFor Fwd idxTy \j -> do
  ...
  where
    idxTy = case dest of
              RawRefTy (TabTy idxBinder _) -> binderType idxBinder

indexDest :: MonadBuilder n m => Dest n -> Atom n -> m (Dest n)
indexDest (Con (TabRef tabVal)) i = appReduce tabVal i
indexDest dest _ = error $ pprint dest

sliceDestDim :: MonadBuilder n m => Int -> Dest n -> Atom n -> Type n -> m (Dest n)
...
sliceDest :: MonadBuilder n m => Dest n -> Atom n -> Type n -> m (Dest n)
sliceDest = undefined
...
destToAtom :: Dest n -> ImpM n (Atom n)
destToAtom dest = fromBuilder $ loadDest dest

destGet :: Dest n -> Atom n -> ImpM n (Dest n)
...
makeAllocDest :: AllocType -> Type n -> ImpM n (Dest n)
makeAllocDest allocTy ty = fst <$> makeAllocDestWithPtrs allocTy ty

makeAllocDestWithPtrs :: AllocType -> Type n -> ImpM n (Dest n, [IExpr n])
...
-- splitDest :: WithDest Block n -> ([WithDest (Decl n l) n], WithDest (Expr n) n, [(Dest n, Atom n)])
-- splitDest = undefined
...
copyDest :: Maybe (Dest n) -> Atom n -> ImpM n (Atom n)
copyDest maybeDest atom = case maybeDest of
  Nothing   -> return atom
  Just dest -> copyAtom dest atom >> return atom

allocDest :: Maybe (Dest n) -> Type n -> ImpM n (Dest n)
...
allocateBuffer :: AddressSpace -> Bool -> BaseType -> IExpr n -> ImpM n (IExpr n)
...
fromBuilder :: Subst e => Builder n (e n) -> ImpM n (e n)
...
intToIndex :: Type n -> IExpr n -> ImpM n (Atom n)
intToIndex ty i = fromBuilder (intToIndexE ty (toScalarAtom i))

indexToInt :: Atom n -> ImpM n (IExpr n)
indexToInt idx = fromScalarAtom <$> fromBuilder (indexToIntE idx)

indexSetSize :: Type n -> ImpM n (IExpr n)
indexSetSize ty = fromScalarAtom <$> fromBuilder (indexSetSizeE ty)

zipTabDestAtom :: HasCallStack => (Dest n -> Atom n -> ImpM n ()) -> Dest n -> Atom n -> ImpM n ()
...
loadAnywhere :: IExpr n -> ImpM n (IExpr n)
loadAnywhere ptr = do
  curDev <- asks curDevice
...
storeAnywhere :: IExpr n -> IExpr n -> ImpM n ()
...
allocateStackSingleton :: IType -> ImpM n (IExpr n)
allocateStackSingleton ty = allocateBuffer Stack False ty (IIdxRepVal 1)

-- === Imp IR builders ===

buildBinOp :: (Atom n -> Atom n -> Builder n (Atom n))
           -> (IExpr n -> IExpr n -> ImpM n (IExpr n))
buildBinOp f x y =
  fromScalarAtom <$> fromBuilder (f (toScalarAtom x) (toScalarAtom y))

iaddI :: IExpr n -> IExpr n -> ImpM n (IExpr n)
iaddI = buildBinOp iadd

isubI :: IExpr n -> IExpr n -> ImpM n (IExpr n)
isubI = buildBinOp isub

imulI :: IExpr n -> IExpr n -> ImpM n (IExpr n)
imulI = buildBinOp imul

idivI :: IExpr n -> IExpr n -> ImpM n (IExpr n)
idivI = buildBinOp idiv

iltI :: IExpr n -> IExpr n -> ImpM n (IExpr n)
iltI = buildBinOp ilt

ieqI :: IExpr n -> IExpr n -> ImpM n (IExpr n)
ieqI = buildBinOp ieq

bandI :: IExpr n -> IExpr n -> ImpM n (IExpr n)
bandI x y = emitInstr $ IPrimOp $ ScalarBinOp BAnd x y

impOffset :: IExpr n -> IExpr n -> ImpM n (IExpr n)
impOffset ref off = emitInstr $ IPrimOp $ PtrOffset ref off

cast :: IExpr n -> BaseType -> ImpM n (IExpr n)
cast x bt = emitInstr $ ICastOp bt x

load :: IExpr n -> ImpM n (IExpr n)
load x = emitInstr $ IPrimOp $ PtrLoad x

memcopy :: IExpr n -> IExpr n -> IExpr n -> ImpM n ()
memcopy dest src numel = emitStatement $ MemCopy dest src numel

store :: HasCallStack => IExpr n -> IExpr n -> ImpM n ()
store dest src = emitStatement $ Store dest src

alloc :: HasCallStack => Type n -> ImpM n (Dest n)
alloc ty = makeAllocDest Managed ty

handleErrors :: ImpM n () -> ImpM n ()
handleErrors m = m `catchError` (const $ emitStatement IThrowError)

emitWhen :: IExpr n -> ImpM n () -> ImpM n ()
...
emitSwitch :: forall n. IExpr n -> [ImpM n ()] -> ImpM n ()
emitSwitch testIdx = rec 0
  where
    rec :: Int -> [ImpM n ()] -> ImpM n ()
...
emitLoop :: NameHint -> Direction -> IExpr n -> (IExpr n -> ImpM n ()) -> ImpM n ()
emitLoop = undefined
...
emitMultiReturnInstr :: ImpInstr n -> ImpM n [IExpr n]
...
emitInstr :: ImpInstr n -> ImpM n (IExpr n)
emitInstr instr = do
...
emitStatement :: ImpInstr n -> ImpM n ()
...
-- emitImpDecl :: ImpDecl -> ImpM n ()
-- emitImpDecl decl = extend $ mempty { envDecls = [decl] }

-- variableScope :: ImpM n Scope
-- variableScope = looks envScope

-- freshVar :: VarP a -> ImpM n (VarP a)
-- freshVar (hint:>t) = do
...
withLevel :: ParallelismLevel -> ImpM n a -> ImpM n a
withLevel level = local (\opts -> opts {curLevel = level })

withDevice :: Device -> ImpM n a -> ImpM n a
...
-- type ImpCheckM i n a = ReaderT (Env (LiftH IType) i n, Device) (Either Err) a

instance Checkable (ImpModule n) where
  checkValid = undefined
  --   checkValid (ImpModule fs) = mapM_ checkValid fs

-- instance Checkable (ImpFunction n) where
...
-- checkImpOp :: IPrimOp i -> ImpCheckM i n IType
-- checkImpOp op = do
...
instance Semigroup (ImpCatEnv n) -- where

  -- (ImpCatEnv a b c d) <> (ImpCatEnv a' b' c' d') =
  --   ImpCatEnv (a<>a') (b<>b') (c<>c') (d<>d')

instance Monoid (ImpCatEnv n) -- where
