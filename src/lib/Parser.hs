...
sourceBlock' =
      proseBlock
  <|> topLevelCommand
  <|> liftM (RunModule . declAsModule) (topDecl <* eolf)
  <|> liftM (RunModule . declAsModule) (instanceDef True  <* eolf)
  <|> liftM (RunModule . declAsModule) (instanceDef False <* eolf)
  <|> liftM (RunModule . declAsModule) (interfaceDef <* eolf)
...
declAsModule :: UDecl S S -> UModule S
declAsModule = undefined

exprAsModule :: UExpr S -> (Name S, UModule S)
exprAsModule e = (v, UModule d (SourceNameMap mempty))
  where
    v = fromRawSourceName "_ans_"
    d = ULet PlainLet (WithSrcH2 (srcPosH e) (nameToPat v), Nothing) e
...
interfaceDef = do
  ...
  (methodNames, methodTys) <- liftM unzip $ onePerLine do
    v <- anyName
    ty <- annot uType
    return (SourceBinder v UnitH, ty)
  let methodNames' = bindersAsBinderList methodNames
  let tyConParams' = toNest tyConParams
  return $ UInterface tyConParams' superclasses methodTys tyConName methodNames'
...
dataDef = do
  ...
  (tyConName, tyConParams) <- tyConDef
  sym "="
  (dataConNames, dataConstructors) <- unzip <$> onePerLine dataConDef
  let def = UDataDef (bindersAsBinderList tyConParams) (H dataConstructors)
  return $ UDataDefDecl def tyConName $ bindersAsBinderList dataConNames

tyConDef :: Parser (UBinder S S, [UAnnBinder S S])
tyConDef = do
  con <- upperName <|> symName
  bs <- many $ label "type constructor parameter" $ do
    v <- lowerName
    ty <- annot containedExpr <|> return tyKind
    return $ SourceBinder v ty
  return (nameAsUBinder con, bs)
  where tyKind = ns $ UPrimExpr $ TCExpr TypeKind

-- TODO: dependent types
dataConDef :: Parser (UBinder S S, EmptyNest UAnnBinder S)
dataConDef = do
  dataConName <- upperName
  bs <- manyNested dataConDefBinder
  return (nameAsUBinder dataConName, Abs bs UnitH)

dataConDefBinder :: Parser (UAnnBinder S S)
dataConDefBinder = annBinder <|> (Ignore <$> containedExpr)

nameAsUBinder :: Name S -> UBinder S S
nameAsUBinder = undefined

bindersAsBinderList ::[BinderP ann S S] -> BinderListP ann S S
bindersAsBinderList = undefined
...
instanceDef isNamed = do
  name <- case isNamed of
    False -> keyWord InstanceKW $> Ignore UnitH
    True  -> keyWord NamedInstanceKW *> (SourceBinder <$> anyName <*> pure UnitH) <* sym ":"
  ...
  let argBinders =
        [UPatAnnArrow (WithSrcH2 Nothing (nameToPat v), Nothing) ImplicitArrow | v <- implicitArgs] ++
        explicitArgs                                                       ++
        [UPatAnnArrow (UnderscoreUPat, Just c)    ClassArrow    | c <- constraints]
...
instanceMethod :: Parser (UMethodDef S)
instanceMethod = do
  v <- anyName
  sym "="
  rhs <- blockOrExpr
  return $ UMethodDef (v:>UnitH) rhs
...
globalEnumPat s = undefined -- ns $ UPatCon (GlobalName s) Empty
...
leafPat =
      (withSrcH2 (symbol "()" $> UPatUnit))
  -- <|> parens (mayPair $ makeExprParser leafPat patOps)
  <|> (withSrcH2 $
          (UPatBinder <$>  (   (nameAsUBinder <$> lowerName)
                           <|> (underscore $> Ignore UnitH)))
...
patPairOp = do
  ...
  if allowed
    then sym "," $> \x y -> joinSrcH2 x y $ UPatPair x y
...
uVariantExpr = withSrcH $ parseVariant expr UVariant UVariantLift
...
joinSrcH2 :: WithSrcH2 a n l -> WithSrcH2 b n l -> c n l -> WithSrcH2 c n l
joinSrcH2 (WithSrcH2 p1 _) (WithSrcH2 p2 _) x = WithSrcH2 (joinPos p1 p2) x
...
manyNested :: Parser (b S S) -> Parser (Nest b S S)
