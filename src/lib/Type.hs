...
module Type (
  ...
  indexSetConcreteSize, projectLength,
...
type TypeEnv n = Bindings n  -- Only care about type payload

data OptionalEnv env = SkipChecks | CheckWith env  deriving Functor
type TypeCheckEnv i o = (SubstEnv i o, OptionalEnv (TypeEnv o, EffectRow o))
type TypeM i o = ReaderT (TypeCheckEnv i o) Except

class PrettyH e => HasType e where
  typeCheck :: e i -> TypeM i o (Type o)

getType :: (HasCallStack, HasType e) => e n -> Type n
getType x = undefined
-- getType x = ignoreExcept $ ctx $ runTypeCheck SkipChecks $ typeCheck x
--   where ctx = addContext $ "Querying:\n" ++ pprint x

checkType :: HasType e => TypeEnv n -> EffectRow n -> e n -> Except ()
checkType env eff x = void $ ctx $ runTypeCheck (idEnv, CheckWith (env, eff)) $ typeCheck x
  where ctx = addContext $ "Checking:\n" ++ pprint x

runTypeCheck :: TypeCheckEnv i o -> TypeM i o a -> Except a
runTypeCheck env m = runReaderT m env

-- -- For debugging
-- traceCheckM :: (HasCallStack, HasVars e, HasType e, Monad m) => e n -> m ()
-- traceCheckM x = traceCheck x (return ())

-- traceCheck :: (HasCallStack, HasVars e, HasType e) => e n -> a -> a
-- traceCheck x y =
...
class Pretty a => Checkable a where
  checkValid :: MonadError Err m => a -> m ()

instance Checkable (Module n) where
...
typeCheckVar :: Var i -> TypeM i o (Type o)
...
--   annTy' <- substTy annTy
--   checkWithEnv \(env, _) ->
--     assertEq annTy' (fstH $ scopeLookup env (varName v)) $ "Annotation on var: " ++ pprint name
--   return annTy'

isDependent :: DataConDef n -> Bool
...
checkCase :: HasType body => Atom i -> [AltP body i] -> Type i -> TypeM i o (Type o)
checkCase e alts resultTy = do
  resultTySubst <- substTy resultTy
  checkWithEnv \_ -> do
    ety <- typeCheck e
    case ety of
      -- TypeCon def params -> do
...
checkApp :: Type o -> Atom i -> TypeM i o (Type o)
checkApp fTy x = do
  Pi piTy <- return fTy
  x |: absArgType piTy
  x' <- substTy x
  let WithArrow arr resultTy = applyAbs piTy x'
...
functionEffs :: Atom n -> EffectRow n
...
-- checkDecl :: TypeEnv -> Decl i l -> TypeM TypeEnv
...
checkArrow :: Arrow i -> TypeM i o ()
checkArrow = mapM_ checkEffRow

infixr 7 |:
(|:) :: HasType e => e i -> Type o -> TypeM i o ()
(|:) x reqTy = do
  ty <- typeCheck x
  checkEq reqTy ty

checkEq :: (Show a, Pretty a, Eq a) => a -> a -> TypeM i o ()
checkEq reqTy ty = checkWithEnv \_ -> assertEq reqTy ty ""

withBinder :: Binder i l -> TypeM l o a -> TypeM i o a
withBinder b m = undefined
...
--   flip runReaderT ir $ mapM_ (checkVariant . withoutArrow) bindings
...
instance CoreVariant (Atom n) where
  checkVariant atom = addExpr atom $ case atom of
    ...
    Lam (Abs b (WithArrow _   body)) -> checkVariant b >> checkVariant body
    Pi  (Abs b (WithArrow arr body)) -> do
      case arr of
...
instance CoreVariant (BinderInfo n) where
...
instance CoreVariant (Decl n l) where
  -- let annotation restrictions?
  checkVariant (Let _ b e) = checkVariant b >> checkVariant e

instance CoreVariant (Block n) where
  checkVariant (Block ds e) = undefined --  mapM_ checkVariant ds >> checkVariant e

instance CoreVariant (Binder n l) where
...
checkEffRow :: EffectRow i -> TypeM i o ()
checkEffRow = undefined
...
declareEff :: Effect o -> TypeM i o ()
declareEff eff = declareEffs $ oneEffect eff

declareEffs :: EffectRow o -> TypeM i o ()
...
substTy :: Subst e => e i -> TypeM i o (e o)
substTy = undefined

checkWithEnv :: ((TypeEnv o, EffectRow o) -> TypeM i o ()) -> TypeM i o ()
...
-- updateTypeEnv :: (TypeEnv o -> TypeEnv o) -> TypeM i o a -> TypeM a
-- updateTypeEnv f m = flip local m $ fmap \(env, eff) -> (f env, eff)

-- extendTypeEnv :: TypeEnv o -> TypeM i o a -> TypeM i o a
-- extendTypeEnv new m = updateTypeEnv (<> new) m

-- withTypeEnv :: TypeEnv o -> TypeM i o a -> TypeM i o a
-- withTypeEnv new m = updateTypeEnv (const new) m

extendAllowedEffect :: Effect o -> TypeM i o () -> TypeM i o ()
extendAllowedEffect eff m = undefined -- updateAllowedEff (extendEffect eff) m

updateAllowedEff :: (EffectRow o -> EffectRow o) -> TypeM i o a -> TypeM i o a
updateAllowedEff f m = undefined -- flip local m $ fmap \(env, eff) -> (env, f eff)

withAllowedEff :: EffectRow o -> TypeM i o a -> TypeM i o a
withAllowedEff eff m = updateAllowedEff (const eff) m

-- === primitive ops and constructors ===

typeCheckTyCon :: TC i -> TypeM i o (Type o)
...
typeCheckCon :: Con i -> TypeM i o (Type o)
typeCheckCon con = case con of
  Lit l -> return $ BaseTy $ litType l
  PairCon x y -> PairTy <$> typeCheck x <*> typeCheck y
  UnitCon -> return UnitTy
  SumAsProd ty tag _ -> tag |:TagRepTy >> substTy ty  -- TODO: check!
  ClassDictHole _ ty -> ty  |: TyKind  >> substTy ty
  IntRangeVal     l h i -> i|:IdxRepTy >> substTy (TC $ IntRange     l h)
  IndexRangeVal t l h i -> i|:IdxRepTy >> substTy (TC $ IndexRange t l h)
  ...
  ConRef conRef -> case conRef of
    ...
    IntRangeVal     l h i ->
      i|:(RawRefTy IdxRepTy) >> substTy (RawRefTy $ TC $ IntRange     l h)
    IndexRangeVal t l h i ->
      i|:(RawRefTy IdxRepTy) >> substTy (RawRefTy $ TC $ IndexRange t l h)
    SumAsProd ty tag _ -> do    -- TODO: check args!
      tag |:(RawRefTy TagRepTy)
      substTy $ RawRefTy ty
    _ -> error $ "Not a valid ref: " ++ pprint conRef
  ParIndexCon t v -> t|:TyKind >> v|:IdxRepTy >> substTy t
  RecordRef _ -> error "Not implemented"

typeCheckRef :: HasType e => e i -> TypeM i o (Type o)
typeCheckRef x = do
  TC (RefType _ a) <- typeCheck x
  return a

checkIntBaseType :: MonadError Err m => Bool -> Type n -> m ()
...
checkFloatBaseType :: MonadError Err m => Bool -> Type n -> m ()
...
checkValidCast :: Type o -> Type o -> TypeM i o ()
...
typeCheckOp :: Op i -> TypeM i o (Type o)
typeCheckOp op = case op of
  TabCon ty xs -> do
    ty |: TyKind
    ty'@(TabTyAbs a) <- substTy ty
  ...
  Select p x y -> do
    ...
    y |: ty
    return ty
  UnsafeFromOrdinal ty i -> ty|:TyKind >> i|:IdxRepTy >> substTy ty
  ...
  ThrowError ty -> ty|:TyKind >> substTy ty
  ThrowException ty -> do
    declareEff ExceptionEffect
    ty|:TyKind >> substTy ty
  CastOp t@(Var _) _ -> t |: TyKind >> substTy t
  CastOp destTy e -> do
    sourceTy <- typeCheck e
    destTy  |: TyKind
    destTy' <- substTy destTy
    checkValidCast sourceTy destTy'
    return destTy'
...
typeCheckHof :: Hof i -> TypeM i o (Type o)
typeCheckHof hof = case hof of
  ...
  RunIO f -> do
    ...
    PairH resultTy' eff' <- lowerNamesErr b $ PairH resultTy eff
...
lowerNamesErr :: (MonadError Err m, HasNames e) => BinderP ann n l -> e l -> m (e n)
lowerNamesErr = undefined

checkRWSAction :: RWS -> Atom i -> TypeM i o (Type o, Type o)
...
checkOpArgType :: MonadError Err m => ArgumentType -> Type n -> m ()
...
checkBinOp :: MonadError Err m => BinOp -> Type n -> Type n -> m (Type n)
...
checkUnOp :: MonadError Err m => UnOp -> Type n -> m (Type n)
checkUnOp op x = do
  checkOpArgType argTy x
  return $ case retTy of
    SameReturn -> x
...
typeReduceBlock :: Scope n -> Block n -> Maybe (Atom n)
typeReduceBlock scope (Block decls result) = undefined
...
--      will get broken.
typeReduceAtom :: Scope n -> Atom n -> Atom n
typeReduceAtom scope x = undefined
