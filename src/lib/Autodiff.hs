...
linearize :: MonadBuilder n m => Atom n -> m (Atom n)
linearize = undefined
...
-- openAbsLin :: MonadBuilder n m => LinSubst i n -> Abs e i
--            -> Atom n -> PrimalT n m (DeferredLinSubst e n)
-- openAbsLin = undefined

runPrimalT :: MonadBuilder n m => PrimalT n m a -> m a
...
tangentFunAsLambda :: MonadBuilder n m => LinA n m (Atom n) -> PrimalT n m (Atom n)
...
applyLinToTangents :: MonadBuilder n m => Atom n -> TangentT n m (Atom n)
...
linearizeAtom :: MonadBuilder n m => LinSubst i n -> Atom i -> LinA n m (Atom n)
linearizeAtom subst atom = case atom of
  ...
  Con con -> linearizePrimCon subst con
...
--   Variant t l i e -> Variant t l i <$> linearizeAtom e
  Con _           -> notImplemented  -- Need to synthesize or look up a tangent ADT
  ...
  BoxedRef _ _ _   -> error "Unexpected ref"
  where emitWithZero = withZeroTangentSubst subst atom


linearizeBlock :: MonadBuilder n m => LinSubst i n -> Block i -> LinA n m (Atom n)
...
linearizeExpr :: MonadBuilder n m
...
linearizeExpr subst expr = case expr of
  ...
  App x i | isTabTy (getType x) -> (App <$> linearizeAtom subst x <*> pureSubst subst i) `bindLin` emit
...
emitUnOp :: MonadBuilder n m => UnOp -> Atom n -> m (Atom n)
emitUnOp op x = emitOp $ ScalarUnOp op x

linearizeOp :: MonadBuilder n m
...
linearizeUnOp :: MonadBuilder n m => LinSubst i n -> UnOp -> Atom i -> LinA n m (Atom n)
...
linearizeBinOp :: MonadBuilder n m => LinSubst i n -> BinOp -> Atom i -> Atom i
               -> LinA n m (Atom n)
...
emitDiscreteOp :: MonadBuilder n m => PrimOp (Atom n) -> LinA n m (Atom n)
...
linearizeHof :: MonadBuilder n m => LinSubst i n -> PrimHof (Atom i) -> LinA n m (Atom n)
...
-- linearizeEff :: MonadBuilder n m => Subst i n -> Atom i -> RWS
--              -> (Atom n -> Hof n)
...
linearizePrimCon :: MonadBuilder n m => LinSubst i n -> Con i -> LinA n  m (Atom n)
linearizePrimCon subst con = case con of
  ...
  SumAsProd ty tg elems -> Con <$> (SumAsProd <$>
    pureSubst subst ty <*>
    pureSubst subst tg <*>
  ...
  where emitWithZero = withZeroTangentSubst subst $ Con con

applyLinSubst :: MonadBuilder n m => LinSubst i n -> e i -> m (e n)
applyLinSubst = undefined

pureSubst ::MonadBuilder n m => LinSubst i n -> e i -> LinA n m (e n) 
pureSubst subst x = LinA do
  x' <- applyLinSubst subst x
  return (x', return x')

withZeroTangentSubst :: MonadBuilder n m => LinSubst i n -> Atom i -> LinA n m (Atom n)
withZeroTangentSubst = undefined

withZeroTangent :: MonadBuilder n m => Atom n -> LinA n m (Atom n)
withZeroTangent = undefined

-- withZeroTangent :: MonadBuilder n m => Atom n -> PirimalT n m (Atom n, TangentT n m (Atom n))

-- withZeroTangent :: MonadBuilder n m => Atom n -> PrimalT n m (Atom n, TangentT n m (Atom n))
...
bindLin :: MonadBuilder n m => LinA n m a -> (a -> m b) -> LinA n m b
bindLin (LinA m) f = LinA $ do
  (e, t) <- m
  x <- lift $ f e
  return (x, t >>= (lift . f))

lookupDerivVar :: MonadBuilder n m => Int -> TangentT n m (Atom n)
lookupDerivVar = undefined

-- isActive :: MonadBuilder n m => Name n -> PrimalT n m Bool
...
-- updateAddAt :: MonadBuilder n m => Atom n -> m (Atom n)
-- updateAddAt x = do
--   ty <- getTypeE x
--   buildLam ty PureArrow $ addTangent x

-- addTangent :: MonadBuilder n m => Atom n -> Atom n -> m (Atom n)
-- addTangent x y = do
--  ty <- getTypeE x
...
-- tangentBaseMonoidFor :: MonadBuilder n m => Type n -> m (BaseMonoid n)
