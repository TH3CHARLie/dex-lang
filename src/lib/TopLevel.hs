...
newtype TopBuilderT n (m :: * -> *) a =
  UnsafeMakeTopBuilderT (StateT (Bindings n, SourceNameMap n) m a)
  deriving (Functor, Applicative, Monad, MonadIO, MonadFail, MonadException)

runTopBuilderT :: Monad m => (forall n. TopBuilderT n m a) -> m a
runTopBuilderT (UnsafeMakeTopBuilderT m) = evalStateT m (emptyScope, SourceNameMap mempty)
...
class Monad m => MonadTopBuilder n m | m -> n where
  extendTopBindings :: WithBindings e n -> m (e n)
...
evalUModuleVal v m = do
  ...
  case evaluated of
    Abs frag sourceMap -> do
      let Just v' = M.lookup v $ fromSourceNameMap sourceMap
      let LetBound PlainLet (Atom x) = sndH $ scopeLookup (extendScope bindings frag) v'
      case lowerNames frag x of
        Nothing -> throw LeakedVarErr ""
        Just x' -> return x'

renameSourceModule :: MonadPasses n m => SourceModule -> m (UModule n)
renameSourceModule = undefined
...
evalUModule sourceMod = do
  ...
  case optimized of
    ...
    _ -> do
      ...
      evalModuleInterp bindings $ applyAbs rest result
...
abstractPtrLiterals block = (Abs (BinderListP Empty []) block, [])
...
instance Monad m => MonadTopBuilder n (TopBuilderT n m)
instance MonadState s m => MonadState s (TopBuilderT n m)
instance MonadReader r m => MonadReader r (TopBuilderT n m)
