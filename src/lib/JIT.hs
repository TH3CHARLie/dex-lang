...
data VoidNS = DoNotUseThisConstructor

type OperandEnv n = Env (LiftH Operand) n VoidNS
type OperandEnvFrag n l = EnvFrag (LiftH Operand) n l VoidNS
data CompileState = CompileState { curBlocks   :: [BasicBlock]
                                 , curInstrs   :: [Named Instruction]
                                 , scalarDecls :: [Named Instruction]
                                 , blockName   :: L.Name
                                 , usedNames   :: S.Set RawName
...
-- compileFunction logger fun@(ImpFunction bs body) = case cc of
...
compileInstr instr = case instr of
  ...
  ICall f@(fname:> LiftH (IFunType cc argTys resultTys)) args -> do
...
makeFunSpec :: IFunVar n -> ExternFunSpec
...
compileLoop d iBinder n compileBody = do
  let loopName = fromString $ "loop_" ++ (show $ getRawName $ binderName iBinder)
  loopBlock <- freshName $ fromString $ loopName
  nextBlock <- freshName $ fromString $ "cont_" ++ loopName
  i <- alloca 1 $ scalarTy $ fromLiftH $ binderAnn iBinder
  ...
  iVal <- load i
  extendOperands (iBinder @> LiftH iVal) $ compileBody
...
outputStreamPtrLName = undefined --  = asLLVMName outputStreamPtrName
...
extendOperands :: OperandEnvFrag n l -> Compile l a -> Compile n a
extendOperands = undefined
-- extendOperands openv = local \env -> env { operandEnv = (operandEnv env) <> openv }

lookupImpVar :: IVar n -> Compile n Operand
lookupImpVar (v:>_) = do
  env <- asks operandEnv
  return case envLookup env v of
    Right x -> fromLiftH x
    Left _ -> error "`Name VoidNS` names shouldn't exist"
