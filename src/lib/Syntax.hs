...
module Syntax (
    ...
    PrimHof (..), LamExpr, PiType, WithSrc (..), srcPos, srcPosH, LetAnn (..),
    BinOp (..), UnOp (..), CmpOp (..), SourceBlock (..), SourceNameMap (..),
    ReachedEOF, SourceBlock' (..), SubstEnv, SubstEnvFrag, ScopedSubstEnv,
    ...
    IScope, BinderInfo (..), Bindings, BindingsFrag, CUDAKernel (..), BenchStats,
    SrcCtx, Result (..), Output (..), OutFormat (..),
    Err (..), ErrType (..), Except, throw, throwIf, modifyErr, addContext,
    addSrcContext, catchIOExcept, liftEitherIO, (-->), (--@), (==>),
    -- boundUVars,
    PassName (..), boundVars, renamingSubst,
    ...
    UMethodDef (..), UPatAnnArrow (..), UBinder, UBinderList, UAnnBinderList,
    UPat, UPat' (..), UModule (..), UDecl (..), UArrow, arrowEff,
    DataDef (..), UDataDef (..),
    NamedDataDef (..), DataConDef (..), Nest (..), toNest,
    WithSrc (..), WithSrcH (..), WithSrcH2 (..), VarP (..),
    subst, scopelessSubst, absArgType, applyAbs,
    -- makeAbs,
...
data DataConRefBinding n l = DataConRefBinding -- (Binder n l) Atom  deriving (Show, Generic)

type AltP (e :: * -> *) = Abs (Nest Binder) e :: * -> *
type Alt = AltP Block                         :: * -> *

infixr 7 :>
data VarP (ann :: * -> *) n = (:>) (Name n) (ann n)  deriving (Show, Eq)

varName :: VarP ann n -> Name n
varName = undefined

type Var = VarP Type :: * -> *
type Binder     = BinderP     Type  :: * -> * -> *
type BinderList = BinderListP Type  :: * -> * -> *

data DataDef n where
  DataDef :: RawName -> BinderList n l -> ListH DataConDef l -> DataDef n

instance Show (DataDef n)
instance Eq   (DataDef n)

data DataConDef n = DataConDef RawName (EmptyNest Binder n)   deriving (Show, Generic)

data NamedDataDef n = NamedDataDef (Name n) (DataDef n)  deriving (Eq, Show)
...
instance Semigroup (SourceNameMap n)
instance Monoid (SourceNameMap n)
...
emptyEvaluatedModule = Abs emptyScopeFrag mempty

instance HasVars Module
instance Subst Module
...
data UDecl n l where
  ...
  UDataDefDecl
    ...
    ->   UBinderList l l'  -- data constructor names
    ->     UDecl n l'
  -- superclasses, dictionary type constructor, types of methods
  UInterface
    :: Nest UAnnBinder n p  -- class parameters
    ->   [UType p]          -- superclasses
    ->   [UType p]          -- method types
    -> UBinder n l          -- class name
    ->   UBinderList l l'   -- method names
    ->     UDecl n l'
  UInstance
    :: Nest UPatAnnArrow n l  -- dictionary args (i.e. conditions)
    ->   UType l              -- dictionary type
    ->   [UMethodDef l]       -- method definitions
    -> UBinder n l'           -- optional instance name
    ->   UDecl n l'

instance Show (UDecl n l)

type UArrow = ArrowP ()
type UType  = UExpr          :: * -> *
type UVar    = VarP UnitH    :: * -> *
type UBinder = BinderP UnitH :: * -> * -> *
type UBinderList = BinderListP UnitH :: * -> * -> *
data UMethodDef n = UMethodDef (UVar n) (UExpr n) deriving (Show, Generic)

type UPatAnn n l = (UPat n l, Maybe (UType n))
data UPatAnnArrow n l = UPatAnnArrow (UPatAnn n l) UArrow

type UAnnBinder     = BinderP     UType  :: * -> * -> *
type UAnnBinderList = BinderListP UType  :: * -> * -> *

data UAlt n where
  UAlt :: UPat n l -> UExpr l -> UAlt n
instance Show n => Show (UAlt n)

data UModule n where
  UModule :: UDecl n l -> SourceNameMap l -> UModule n

instance Show (UModule n)

type UPat = WithSrcH2 UPat'
data UPat' n l where
  UPatUnit   :: UPat' n n
  UPatBinder :: UBinder n l             -> UPat' n l
...
instance (Show n, Show l) => Show (UPat' n l)

instance IsString (UExpr' n) where
  fromString s = undefined -- UVar $ Name SourceName (fromString s) 0 :> ()

pattern UnderscoreUPat :: UPat n n
pattern UnderscoreUPat = WithSrcH2 Nothing (UPatBinder (Ignore UnitH))
...
getBaseMonoidType :: Type n -> Type n
...
outputStreamPtrName = undefined -- GlobalName "OUT_STREAM_PTR"

initBindings :: Bindings SourceNS
...
type SourceModule = UModule SourceNS
...
type IBinder     = BinderP     ITypeH
type IBinderList = BinderListP ITypeH
type IVar = VarP ITypeH
type IType = BaseType
type Size = IExpr

type IFunVar = VarP (LiftH IFunType)
data IFunType = IFunType CallingConvention [IType] [IType] -- args, results
                deriving (Show)

type ITypeH    = LiftH IType
type IFunTypeH = LiftH IFunType
...
data ImpModule n where
  ImpModule :: BinderListP IFunTypeH n l  -- local function names
...
data BinderInfo n =
 ...
 | MethodName  (NamedDataDef n) Int
 | DataConName (NamedDataDef n) Int
 | TypeConName (NamedDataDef n)
   deriving (Show, Generic)

type SubstEnv = Env Atom
type SubstEnvFrag = EnvFrag Atom
type Bindings     = ScopeP     (PairH Type BinderInfo)
type BindingsFrag = ScopeFragP (PairH Type BinderInfo)

type Scope = Bindings  -- when we only care about the names, not the payloads
type ScopedSubstEnv i o = (SubstEnv i o, Bindings o)

scopelessSubst :: Subst e => SubstEnv i o -> e i -> e o
scopelessSubst = undefined
...
class HasVars e where
  freeVars :: e n -> NameSet n

class HasVars e => Subst e where
  subst :: ScopedSubstEnv i o -> e i -> e o

class BindsVars (b :: * -> * -> *) where
  boundVars :: b n l -> BinderListP UnitH n l
  renamingSubst :: ScopedSubstEnv i o -> b i l
                -> (forall l'. (b o l', ScopedSubstEnv l' o)  -> a)
                -> a
...
applyAbs :: Subst e => Abs Binder e n -> Atom n -> e n
applyAbs (Abs b body) x = undefined  -- scopelessSubst (b@>x) body

applyNaryAbs :: Subst e => Abs (Nest Binder) e n -> [Atom n] -> e n
...
-- makeAbs :: HasVars a => Binder -> a -> Abs Binder e n
...
unitSubstFrag :: Binder n l -> SubstEnvFrag n l any
unitSubstFrag b = undefined
...
instance HasNames Atom
instance HasNames Expr
instance HasNames EffectRow

instance HasVars Atom
instance Subst   Atom

instance HasVars Expr
instance Subst   Expr

instance HasVars Block
instance Subst   Block

instance (HasVars e) => HasVars (WithArrow e)
instance (Subst e) => Subst (WithArrow e)
