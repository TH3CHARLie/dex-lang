...
module Builder (emit, emitAnn, emitOp, buildDepEffLam, buildPi,
                ...
                select, substBuilder, emitUnpack, getUnpacked,
                ...
                singletonTypeVal, scopedDecls, applyPreludeFunction,
...
-- TODO: figure out what the fundamental minimal set of basis functions is
-- TODO: `HasNames` constraints
class Monad m => MonadBuilder n m | m -> n where
  withBindings :: (Bindings n -> e n) -> m (e n)
  emitAnn :: LetAnn -> Expr n -> m (Atom n)
  scopedDecls :: m (e n) -> m (WithDecls e n)
  buildAbs :: Type n -> (Var n -> m (e n)) -> m (Abs Binder (WithDecls e) n)
  getNameHint :: m RawName

instance Monad m => Functor        (BuilderT m n)
instance Monad m => Applicative    (BuilderT m n)
instance Monad m => Monad          (BuilderT m n)
instance Monad m => MonadBuilder n (BuilderT m n)

instance MonadBuilder n m => MonadBuilder n (ReaderT r m)



lookupBindings :: MonadBuilder n m => Name n -> m (Type n, BinderInfo n)
...
-- runBuilder m scope = undefined -- runIdentity $ runBuilderT m scope
...
emitBlock block = emitBlockRec idEnv block
...
mkBinaryEffFun rws v ty body = do
  eff <- getAllowedEffects
  buildLam TyKind PureArrow \r@(Var (rName:>_)) -> do
...
substBuilder env x = withBindings \scope -> subst (env, scope) x
...
-- class Monad m => MonadBuilder n m where
...
-- instance MonadBuilder n m => MonadBuilder (ReaderT r m) where
...
-- instance MonadBuilder n m => MonadBuilder (StateT s m) where
--   builderLook = lift builderLook
--   builderExtend x = lift $ builderExtend x
--   builderScoped m = do
--     s <- get
...
-- instance (Monoid env, MonadBuilder n m) => MonadBuilder (CatT env m) where
--   builderLook = lift builderLook
--   builderExtend x = lift $ builderExtend x
--   builderScoped m = do
--     env <- look
...
-- instance (Monoid w, MonadBuilder n m) => MonadBuilder (WriterT w m) where
--   builderLook = lift builderLook
--   builderExtend x = lift $ builderExtend x
--   builderScoped m = do
...
--     name <- ask
--     env <- look
--     ((ans, env'), scopeEnv) <- lift $ lift $ scoped $ runCatT (runReaderT mame) env
--     extend env'
--     return (ans, scopeEnv)
...
-- getNameHint :: MonadBuilder n m => mame
-- getNameHint = do
--   tag <- fst <$> builderAsk
